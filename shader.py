...
def FHDMenu(shader_dir):
    ...
    #    quality_choice = GUI_OPTS['upscale']['sh#ader_quality_choice']
...
def cpu_shader(fn, width, height, shader, ten_bit, outname, files=[]):
    ...
    print("Using the following shaders:")
    print(str_shaders)
    print("Encoding with preset: " +  str(x264_preset))
    import time
    time.sleep(3)
    #clear()
    ...
    if len(files) == 0:
        subprocess.call([
            ...
            "--glsl-shaders=" + str_shaders,
            "--ovc=libx264",
            '--ovcopts=preset=' + str(x264_preset) + ',level=5.1,crf=' + str(crf) + ',aq-mode=3,psy-rd=1.0,bf=6',
            '--no-audio',
            '--o=' + outname
        ])
    else:
        ...
        for f in files:
            ...
            subprocess.call([
                ...
                "--glsl-shaders=" + str_shaders,
                "--ovc=libx264",
                '--ovcopts=preset=' + str(x264_preset) + ',level=5.1,crf=' + str(crf) + ',aq-mode=3,psy-rd=1.0,bf=8',
                '--no-audio',
                '--o=' + os.path.join(outname, name)
            ]) 
            i = i + 1      
