...
if args.png:
    ...
    '-f', 'rawvideo',
    '-vcodec', 'rawvideo',
    '-pix_fmt', 'bgr24',                # OpenCV は BGR
    '-s', f'{w}x{h}',
    '-r', str(int(np.round(args.fps))),
    '-i', '-',                          # stdin から読み込む
    '-c:v', 'libx264',                  # 出力コーデック（例）
    '-preset', 'veryfast',
    '-crf', '18',
    '-pix_fmt', 'yuv420p',              # 互換性のため出力ピクセル形式
    vid_out_name
    ]
    # バッファを大きめにしておくと安定することがある
...
"""
# --- clear_write_buffer を ffmpeg 用に書き換え ---
def clear_write_buffer(user_args, write_buffer, ffmpeg_proc=None):
    ...
    while True:
        ...
        if user_args.png:
            ...
            # item は OpenCV の BGR 配列（H,W,3）
            # ffmpeg に渡す場合はそのままバイト列で渡せる（pix_fmt=bgr24 を指定しているため）
            try:
                ffmpeg_proc.stdin.write(item.tobytes())
            except BrokenPipeError:
                # ffmpeg が何らかの理由でた場合の保険
                break
